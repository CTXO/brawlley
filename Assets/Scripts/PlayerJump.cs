...
public class PlayerJump : MonoBehaviour
{
    ...
    [SerializeField, Range(2f, 10f)][Tooltip("Maximum jump height")] private float jumpHeight = 7.3f;
    [SerializeField, Range(0.2f, 1.25f)][Tooltip("How long it takes to reach that height before coming back down")] private float timeToJumpApex;
    [SerializeField, Range(0f, 5f)][Tooltip("Gravity multiplier to apply when going up")] private float upwardMovementMultiplier = 1f;
    [SerializeField, Range(1f, 10f)][Tooltip("Gravity multiplier to apply when coming down")] private float downwardMovementMultiplier = 6.17f;
    ...
    [SerializeField, Range(1f, 10f)][Tooltip("Gravity multiplier when you let go of jump")] private float jumpCutOff;
    [SerializeField][Tooltip("The fastest speed the character can fall")] private float speedLimit;
    ...
    private void SetPhysics()
    {
        ...
        playerRb.gravityScale = (newGravity.y / Physics2D.gravity.y) * gravMultiplier;
    }
    ...
    private void CalculateGravity()
    {
        ...
        playerRb.linearVelocity = new Vector3(velocity.x, Mathf.Clamp(velocity.y, -speedLimit, 100));
    }
    ...
    private void DoAJump()
    {
        ...
        if (onGround || (coyoteTimeCounter > 0.03f && coyoteTimeCounter < coyoteTime) || canJumpAgain)
        {
            ...
            jumpSpeed = Mathf.Sqrt(-2f * Physics2D.gravity.y * playerRb.gravityScale * jumpHeight);
            ...
            {
                jumpSpeed += Mathf.Abs(playerRb.linearVelocity.y);
            }
    ...
    public void BounceUp(float bounceAmount)
    {
        ...
        playerRb.AddForce(Vector2.up * bounceAmount, ForceMode2D.Impulse);
    }
}
